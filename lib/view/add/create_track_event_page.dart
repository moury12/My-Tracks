...
class CreateTrackEventScreen extends StatefulWidget {
  static const String routeName = '/create-track-event';

  CreateTrackEventScreen({super.key});

  @override
  State<CreateTrackEventScreen> createState() => _CreateTrackEventScreenState();
}

class _CreateTrackEventScreenState extends State<CreateTrackEventScreen> {
  ...
  void initState() {
    ...
    if (args is SingleTrackModel) {
      editing = true;
      final trackModel = args;
      CreateTrackEventController.to.trackNameController.value.text =
          trackModel.trackName ?? "No Data Found";
      CreateTrackEventController.to.trackLocationController.value.text =
          trackModel.address ?? "No Data Found";
      CreateTrackEventController.to.trackDescriptionController.value.text =
          trackModel.description ?? "No Data Found";
    } else {
      argument = args;
    }
  }
...
*/
                  ...
                  editing ? Obx(() {
                    return CustomButton(
                      isLoading: CreateTrackEventController.to
                          .isLoadingUpdateTrack.value,
                      onTap: () {
                        CreateTrackEventController.to.trackId.value = (Get.arguments as SingleTrackModel).sId??"";
                        CreateTrackEventController.to.updateTrackCall();
                      }, title: AppStaticString.editTrack,);
                  }) : Obx(() {
                    return CustomButton(
